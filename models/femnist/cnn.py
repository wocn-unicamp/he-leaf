...
class ClientModel(Model):
    ...
    def create_model_para_batch_senstividade(self):
        """Model function for CNN."""
        features = tf.placeholder(
            tf.float32, shape=[None, IMAGE_SIZE * IMAGE_SIZE], name='features')
        labels = tf.placeholder(tf.int64, shape=[None], name='labels')
        ...
        # --- Aquí está la loss por muestra ---
        loss_per_sample = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=labels, logits=logits)  # shape: [batch_size]
        ...
        # Asigna el tensor para acceso posterior (¡importante!)
